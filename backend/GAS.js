...
function doPost(e) {
    try {
        ...
        if (action === 'logEvent') {
            ...
            sendAutoReplyToUser(body.details);
            ...
            console.log("Action is 'user_signup'.");
            const ipAddress = e.source ? e.source.remoteAddress : 'N/A';

            // Log the signup attempt
            logEventToSheet({
                type: 'user_signup_attempt',
                assistant: 'Auth System',
                details: body.details
            }, ipAddress);

            try {
                sendSignupNotificationEmail(body.details);
                ...
                logEventToSheet({
                    type: 'email_error',
                    assistant: 'Auth System',
                    details: { error: emailError.toString(), context: 'user_signup' }
                }, ipAddress);
            }
...
/**
 * Sends email notification when new user signs up
 * @param {object} details - Contains email and method (Email/Password or Google)
 */
function sendSignupNotificationEmail(details) {
        ...
        <div style="font-family: Arial, sans-serif; max-width: 600px;">
            ...
            <table style="width: 100%; border-collapse: collapse;">
                <tr>
                    ...
                    <td style="padding: 8px; border-bottom: 1px solid #e5e7eb;">${email || 'Unknown'}</td>
                </tr>
                <tr>
                    <td style="padding: 8px; border-bottom: 1px solid #e5e7eb;"><strong>Method:</strong></td>
                    <td style="padding: 8px; border-bottom: 1px solid #e5e7eb;">${method || 'Unknown'}</td>
                </tr>
                <tr>
